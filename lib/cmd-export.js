...
const DEFAULT_OUTPUT = '地图草稿名'; // 默认输出目录名（用草稿名）
...
function parseArgs(argv) {
  const opts = {
    map: null, output: null, compact: false, listTypes: false, list: false,
    interactive: false, quiet: false, help: false, noColor: false, root: null, unknown: null,
  };
  const pos = [];
  for (let i = 0; i < argv.length; i++) {
    const a = argv[i];
    if (a === '-h' || a === '--help') opts.help = true;
    else if (a === '-c' || a === '--compact') opts.compact = true;
    else if (a === '-t' || a === '--list-types') opts.listTypes = true;
...
function helpText() {
  const lines = [
    '',
    `${c.bold('蛋仔派对 PC 编辑器 · UI 数据导出工具')}`,
    '',
    `${c.dim('从地图工程目录中读取 eui.mm，解压 zstd + 解码 MessagePack，')}`,
    `${c.dim('输出结构清晰、字段可读、层级嵌套明确的 UI 树 JSON，')}`,
    `${c.dim('供 AI / 脚本直接“看懂”编辑器里搭好的界面。')}`,
    '',
    `${c.cyanBold('用法')}`,
    ...
    '',
    `${c.cyanBold('参数')}`,
    `  <草稿名称>       按地图草稿名称获取（支持片段，如“我的地图”或“我的”）`,
    `  <地图目录>       地图工程目录：FS 含 eui.mm 的 “xxx__pc” 文件夹 / SE 含 euidata.mm 的文件夹`,
    `                   （也支持：地图工程根目录 / base64 地图 id / 路径片段）`,
    `  自动扫描：editor_maps（FS）、help_build_gmps / joint_construction_gmps / se_maps（含 SE）`,
    `  -l, --list       列出所有地图草稿名称后退出`,
    `  -i, --interactive 强制进入交互选择（脚本/管道也可用）`,
    `  -r, --root <path> 手动指定编辑器根目录（自动探测失败时用）`,
    `  -o, --output <dir> 输出目录（直接指定）；未指定时按优先级取：`,
    `                   EGGY_EUI_OUTPUT 环境变量 > eui config export-dir > 当前目录`,
    `                   （后两者为基础目录，实际输出 <基础目录>/<草稿名>/）`,
    `  -c, --compact    输出压缩 JSON（默认美化缩进 2 格）`,
    ...
    '',
    `${c.cyanBold('示例')}`,
    `  eui export                              # 弹出草稿列表供选择`,
    `  eui export 我的地图                      # 按草稿名称获取`,
    `  eui export -l                           # 查看全部草稿名称`,
    ...
    `  每个节点 = 一个对象：id/name/type/type_code/type_zh + 可读分组：`,
    `  · layout     布局几何：anchor/pos/size/rotation/opacity/flip 等`,
    `  · visibility 显隐规则；events 触摸与显隐事件；behavior 交互`,
    `  · text/image/panel/background 控件专属外观（颜色已转 #hex）`,
    `  · children   子节点数组（顺序 = 渲染/绘制顺序）`,
    `  · other      未归类的原始字段（保留完整信息，值已可读化）`,
    '',
    `${c.dim('说明：eui.mm 内部为 Zstandard 压缩 + MessagePack 序列化；')}`,
    `${c.dim('草稿名称取自各工程 desc.mm 的 map_name 字段。本工具零依赖。')}`,
  ];
  return lines.join('\n');
}
...
function pickProject(root, projects, token) {
  ...
  const hit = projects.filter((d) => d === token || d.startsWith(token) || d.includes(token));
...
function resolveMapDir(input, cwd) {
  ...
  for (const root of maps.findMapRoots()) {
    ...
    const hit = projects.filter((d) => d === input || d.startsWith(input) || d.includes(input));
    if (hit.length === 1) return { dir: path.join(root, hit[0]), folder: hit[0] };
...
async function run(argv) {
  ...
  if (!opts.map) {
    ...
    resolved = await resolveInput(opts.map, cwd);
  }
...
module.exports = { run, parseArgs, helpText, printTypes };
